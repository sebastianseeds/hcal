...
int cosCal_v3(int run = 1725, int event = -1){
  ...
  cout << "Fitting pedestal histos with Gaussian and extracting mean for event pedestal subtraction.." << endl;
  for(int r=0; r<kNrows; r++){
    for(int c=0; c<kNcols; c++){
  ...
  cout << "Pedestal subtracting and processing event pulses into ADC and TDC spectra histos.." << endl;
  for (int i = gCurrentEntry; i < T->GetEntries(); i++){ 
