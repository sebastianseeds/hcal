...
void hcal_signal_centering( int run = -1 ){
  ...
  TFile *fout = new TFile( Form("outfiles/SignalCentering_%d_v2.root",run), "RECREATE" );
   
  // Declare histograms and arrays
  TH1D *gHistos[kNrows][kNcols];
  TH1D *gATimeSpec[kNrows][kNcols];
  TH1D *ATimeDist_raw = new TH1D( "ADC time over all channels - samps only", "ADCTime_raw", totSample, minSample, maxSample );
  ...
  if( !T ) {
    ...
    T->SetBranchStatus( "bb.tdctrig.*", 1 );
  ...
  while( progress<1.0 ){
	...
	if(hcalt::TDCT_id[ihit]==0) hcal_time=hcalt::TDCT_tdc[ihit];
      ...
      // Cut on BB/HCal trigger coincidence (510 from before, 75ns adjustment goes to 435 and 585), std cut 20ns
      //if( fabs(diff-510.)< 145 ){
