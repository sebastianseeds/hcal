...
void pmtqe_ADC_test(Int_t run=301, Bool_t saveCanvas = kFALSE)
{
  ...
  for(Int_t r = 0; r < kNrows; r++) {
    for(Int_t c = 0; c < kNcols; c++) {
      for(Int_t iled = 0; iled < kNLED; iled++) {
        ...
        int ledbit = (int) (log2(iled+1)+1);  //Convert led bit binary from root tree to led on number
