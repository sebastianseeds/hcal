...
void hcal_e_cal( int run = -1 ){
  ...
  int min_events_per_cell=100; //require minimum amount of events to populate matrix elements
  cout << "Enter minimum events required per cell." << endl;
  cin >> min_events_per_cell;

  int max_events_per_cell=4000; //reject events which overpopulate matrix elements 
  cout << "Enter maximum events allowed per cell." << endl;
  cin >> max_events_per_cell;

  int debug = 0; //Comment to 1 to enable all cout statements
  ...
  //Declare outfile
  TFile *fout = new TFile( Form("%s/hcalECal_%d.root", getenv("OUT_DIR"), run), "RECREATE" );
  ...
  TH1F *badCvChannel = new TH1F( "badCvChannel", "Bad Cell vs Channel", kNcols*kNrows, 0, kNcols*kNrows );
  TH1F *CRatiovChannel = new TH1F( "CRatiovChannel", "Constant Ratio (new/old) vs Channel", kNcols*kNrows, 0, kNcols*kNrows );
  ...
  //int min_events_per_cell=10; //require at least 100 events to calculate a new calibration constant for the cell:
  ...
  */
  ...
  while( T->GetEntry( hitlist[nevent++] ) ){ 
    ...
    if( best >= 0 && ncellclust >= 1 && saturated == 0 ){ // require at least 2 hits in this cluster to use as a "good" event for calibration purposes:
      ...
      if( rowmax <= 0 || rowmax >= 24 || colmax <= 0 || colmax >= 12 ){
	edge_max = true;
      }
	...
	for(int ihit=0; ihit<ncellclust; ihit++ ){ //outer loop over all cells in the cluster: 
	  ...
	  if(nevents[cell_i] > max_events_per_cell) continue; //Added to keep statistics similar across all channels
  ...
  */
  ...
  for(int i=0; i<ncell; i++){
    ...
    NEVvChannel->SetBinContent(i+1,nevents[i]);
    ...
    if( nevents[i] >= min_events_per_cell ){
      ...
      //cout << "solution[] " << solution[i] << " solution() " << solution(i) << endl;

      CRatiovChannel->SetBinContent(kNcols*r+c+1,cRatio[r][c]);
	...
	icol = i%kNcols;
      ...
      //Constants_rowcol->SetBinError( icol+1, irow+1, E_targ*sqrt(fabs(M(i,i)))/sqrt(nevents[i]));
  ...
  */
  ...
  badCvChannel->SetTitle( "Bad Cell vs Channel" );
