...
void eng_cal_BBCal(const char *configfilename, int iter=1)
{
  ...
  for(Int_t shrow = 0; shrow<kNrowsSH; shrow++){
	...
	gainRatioSH_outData << CoeffR(cell) << " " << endl;
      }else{
  ...
  for(Int_t psrow = 0; psrow<kNrowsPS; psrow++){
    for(Int_t pscol = 0; pscol<kNcolsPS; pscol++){
      ...
      if(!badcells[cell]){
	h_coeffRatio_PS->Fill( ncell-cell, CoeffR(cell) );
	h_coeffChan_PS->Fill( ncell-cell, CoeffR(cell)*oldCoeff );
	h_neventChan_PS->Fill( ncell-cell, events_per_cell[cell] );
	h_coeffDV_PS->Fill( pscol+1, psrow+1, CoeffR(cell)*oldCoeff );
	h_oldCoeffChan_PS->Fill( ncell-cell, oldCoeff );

	cout << CoeffR(cell) << "  ";
	adcGainPS_outData << CoeffR(cell)*oldCoeff << " ";
	gainRatioPS_outData << CoeffR(cell) << " " << endl;
      }else{
	h_coeffRatio_PS->Fill( ncell-cell, 1. );
	h_coeffChan_PS->Fill( ncell-cell, oldCoeff );
	h_neventChan_PS->Fill( ncell-cell, events_per_cell[cell] );
