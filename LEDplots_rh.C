...
void LEDplots_rh(int run1 = 1686, int run2 = 1687, int run3 = 1688, int run4 = 1689, int run5 = 1691, int run6 = 1692, int run7 = 1693, int run8 = 1694, int run9 = 1695, int run10 = 1696){ 
  ...
  for(int i=0; i<runT; i++){
	...
	for(int l=1; l<kNLED; l++){ //Ignoring led = 0, as no led is on for this setting
	  ...
	  //if(!h2) cout << "MISSING HISTOGRAM AT ROW " << r << ", COL " << c << "." << endl;
	  ...
	  if(h2){
	    ...
	    //cout << "row " << r << " col " << c << " led " << l << " maxMean " << maxMean[r][c][l][i] << endl;
  ...
  ofstream outFile;
  outFile.open("setFiles/alphas_rh.txt");
  time_t now = time(0);
  char *dt = ctime(&now);
  outFile << "#Alpha parameters obtained from LEDplots_rh on " << dt << "#" << endl;
  ...
  for(int r=0; r<kNrows; r++){ 
	...
	for(int l=1; l<kNLED; l++){
	  NPE_f2[r][c][run][l]=NPE_f1[r][c][l][run];
  ...
  for(int c=0; c<144; c++){
    ...
    //cout << "r " << rval << " c " << cval << " maxAlpha entries " << maxAlpha[rval][cval]->GetEntries() << endl;
  ...
  for(int r=0; r<12; r++){
	...
	for(int run=0; run<runT; run++){ //Calculate useful averages where data from runs with consistently non-linear response or runs with the LED off are ignored.

	  if(l==1&&run<10) NPEaveL1+=NPE_f1[r][c][l][run];
	  if(l==2&&run<9) NPEaveL2+=NPE_f1[r][c][l][run];
	  if(l==3&&run<7) NPEaveL3+=NPE_f1[r][c][l][run];
	  if(l==4&&run<4) NPEaveL4+=NPE_f1[r][c][l][run];
	  if(l==5&&run<2) NPEaveL5+=NPE_f1[r][c][l][run];
	...
	for(int run=0; run<runT; run++){	
	  ...
	  if(NPE_f1[r][c][l][run]<1.2*avgNPE[12*r+c][l]&&NPE_f1[r][c][l][run]>0.8*avgNPE[12*r+c][l]){
	...
	*/
	...
	//cout << "Plateau region for row " << r << ", col " << c << ", and LED " << l << " is from " << HVplatMin << "-" << HVplatMax << "." << endl;
