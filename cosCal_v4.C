...
// Main
int cosCal_v4(int run = 1725, int event = -1){
  ...
  for( int r=0; r<kNrows; r++ ){
    for( int c=0; c<kNcols; c++ ){  
      ...
      gPedSpec[r][c] = new TH1F( Form( "Pedestal Spect R%d C%d", r, c ), Form( "Pedestal Spect R%d C%d", r, c ), 200, 120, 320 ); // Empirical limits
  ...
  if( !T ) { 
    T = new TChain( "T" );
    T->Add( Form( "rootFiles/cosmic/fadc_f1tdc_%d_*.root", run ) );
