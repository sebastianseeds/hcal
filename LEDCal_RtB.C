...
void processEvent(int entry = -1){
  ...
  if(ledNumber == 1){
	...
	if(histos[r][c][ledNumber]->GetBinContent(histos[r][c][ledNumber]->GetMaximumBin())>10){
	  //Get return to baseline parameter (in bins (ADC samples, 4ns per sample))
	  double baseline = 0.0;
	  //int return = 0.0;
	  for(int s = 0; s < 4; s++){ //Get baseline from first four bins
	    
	    baseline += histos[r][c][ledNumber]->GetBinContent(s+1-minSample);
	  ...
	  for(int s = histos[r][c][ledNumber]->GetMaximumBin(); s < totSample; s++){
	    double binContent = histos[r][c][ledNumber]->GetBinContent(s);

	    //cout << pedSigma[r][c] << endl;
	    if(histos[r][c][ledNumber]->GetBinContent(histos[r][c][ledNumber]->GetMaximumBin())>(baseline+15*pedSigma[r][c])){

	    
	      if(binContent<baseline+3*pedSigma[r][c]){
		//cout << s-histos[r][c]->GetMaximumBin() << endl;
		retToBase[r][c]->Fill(s-histos[r][c][ledNumber]->GetMaximumBin());
		
		if((s-histos[r][c][ledNumber]->GetMaximumBin())<3&&Draw2==0&&Draw1<20){
		  histos[r][c][ledNumber]->SetTitle(Form("RetToBase < 5 R%d C%d LED%d",r,c,ledNumber));
		  histos[r][c][ledNumber]->Draw();
		  HistosFile->cd();
		  histos[r][c][ledNumber]->SetTitle(Form("RetToBase < 5 R%d C%d LED%d",r,c,ledNumber));
		  histos[r][c][ledNumber]->GetYaxis()->SetTitle("RAU");
		  ...
		  histos[r][c][ledNumber]->GetXaxis()->SetTitle("fADC Channel");
		  histos[r][c][ledNumber]->GetXaxis()->CenterTitle();
		  histos[r][c][ledNumber]->Write(Form("RetToBase < 5 R%d C%d LED%d",r,c,ledNumber));
		  histos[r][c][ledNumber]->Draw("AP");
		  Draw1++;
		  Draw2++;
		}
...
int LEDCal_RtB(int run = 1205, int event = 1000)  //Start after LEDs warm up ~1000 events
{
  ...
  //Declare outfile
  TFile *ledAggFile = new TFile(Form("outFiles/ledRtBFile_run%d.root",run),"RECREATE");
