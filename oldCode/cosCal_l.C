  ...
  for(int r=0; r<kNrows; r++){
    for(int c=0; c<kNcols; c++){
      ...
      */
      ...
      //if(r==6&&c==1)landauFitMax[r][c]->SetParameter(1,100);
      ...
      //if(r==6&&c==1)landauFitMaxTDC[r][c]->SetParameter(1,100);
	...
	if(landauFitInt[r][c]->GetParameter(1)>0&&landauFitInt[r][c]->GetParameter(1)<2000){
	  fitIntX_l = landauFitInt[r][c]->GetParameter(1);
	  landauFitInt[r][c]->SetLineColor(kBlue);
	}else{
	  //landauFitInt[r][c]->SetParameter(1,61);
	  //PMTIntSpec[r][c]->Fit(landauFitInt[r][c],"+RQ");
	  //if(landauFitInt[r][c]->GetParameter(1)>0&&landauFitInt[r][c]->GetParameter(1)<2000){
	  fitIntX_l = PMTIntSpec[r][c]->GetMean();
	...
	PMTIntSpec[r][c]->SetTitle(Form("Int ADC Spect R%d C%d MaxX_l%f",r,c,fitIntX_l));
	...
	if(landauFitMax[r][c]->GetParameter(1)>0&&landauFitMax[r][c]->GetParameter(1)<2000){
	  ...
	  fitMaxX_l = PMTMaxSpec[r][c]->GetMean();
	...
	PMTMaxSpec[r][c]->SetTitle(Form("Max ADC Spect R%d C%d MaxX_l%f",r,c,fitMaxX_l));
	...
	if(landauFitIntTDC[r][c]->GetParameter(1)>0&&landauFitIntTDC[r][c]->GetParameter(1)<2000){
	  ...
	  TDCfitIntX_l = PMTIntSpecTDC[r][c]->GetMean();
	...
	PMTIntSpecTDC[r][c]->SetTitle(Form("Int ADC Spect TDCcut R%d C%d MaxX_L%f",r,c,TDCfitIntX_l));
	...
	if(landauFitMaxTDC[r][c]->GetParameter(1)>0&&landauFitMaxTDC[r][c]->GetParameter(1)<400){
	  ...
	  TDCfitMaxX_l = PMTMaxSpecTDC[r][c]->GetMean();
	  landauFitMaxTDC[r][c]->SetLineColor(kRed);
	}
	if(TDCfitMaxX_l>2000)cout<<r<<"  "<<c<<endl;
	...
	PMTMaxSpecTDC[r][c]->SetTitle(Form("Max ADC Spect TDCcut R%d C%d MaxX_l%f",r,c,TDCfitMaxX_l));
