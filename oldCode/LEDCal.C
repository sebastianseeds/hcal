...
double NPE[kNrows][kNcols][kNLED];
double LED[kNLED-1];
...
int saturation[kNrows][kNcols][kNLED]={0};
...
void processEvent(int entry = -1){
  ...
  for(r = 0; r < kNrows; r++) {
	...
	PMTIntSpec[r][c][ledNumber]->Fill(0.0); //When the LED is off, bin to zero (not useful data)
	PMTMaxSpec[r][c][ledNumber]->Fill(0.0);
      }
...
int LEDCal(int run = 1205, int event = 1000)  //Start after LEDs warm up ~1000 events
{
  ...
  for(int r=0; r<kNrows; r++){
	...
	PMTIntSpec[r][c][l] = new TH1F(Form("Int ADC Spect R%d C%d LED%d",r,c,l),Form("Int ADC Spect R%d C%d LED%d",r,c,l),25,0,0);
	PMTIntSpec[r][c][l]->GetXaxis()->SetTitle("sRAU");
	PMTIntSpec[r][c][l]->GetXaxis()->CenterTitle();

	PMTMaxSpec[r][c][l] = new TH1F(Form("Max ADC Spect R%d C%d LED%d",r,c,l),Form("Max ADC Spect R%d C%d LED%d",r,c,l),25,0,0);
  ...
  */
  ...
  for(int r=0; r<kNrows; r++){
	...
	if(l!=0){
	  ...
	  if(saturation[r][c][l]<0.1*PMTMaxSpec[r][c][l]->GetEntries()){  //If more than 10% of total events saturate, ignore point (bin to zero)
