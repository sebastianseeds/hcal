...
const int chan = 288; //Number of active channels
...
double avgNPE[kNrows*kNcols][kNLED];
double SDNPE[kNrows*kNcols][kNLED];
double channel[kNrows*kNcols];
double SplatMin[kNrows*kNcols][kNLED]={0};
double SplatMax[kNrows*kNcols][kNLED]={0};
double VplatMin[kNrows*kNcols][kNLED]={0};
double VplatMax[kNrows*kNcols][kNLED]={0};
double Sdif[kNLED][kNrows*kNcols]={0};
double Smean[kNLED][kNrows*kNcols]={0};
double Vdif[kNLED][kNrows*kNcols]={0};
double Vmean[kNLED][kNrows*kNcols]={0};

double LED[5] = {1,2,3,4,5}; //Array for plotting

TH1F *maxVar[kNLED];
TH1F *maxAlpha[kNrows][kNcols];
TH1F *NPEhist[kNrows][kNcols][kNLED];
...
void LEDplots_v3(int run1 = 1288, int run2 = 1289, int run3 = 1290, int run4 = 1291, int run5 = 1292, int run6 = 1293, int run7 = 1294, int run8 = 1297, int run9 = 1300, int run10 = 1301){ 
  ...
  ofstream outFile;
  outFile.open("setFiles/alphas_lh.txt");
  time_t now = time(0);
  char *dt = ctime(&now);
  ...
  for(int r=12; r<kNrows; r++){
    for(int c=0; c<kNcols; c++){
      for(int l=1; l<kNLED; l++){
	NPEhist[r][c][l] = new TH1F(Form("R%dC%d NPE LED%d",r,c,l),Form("R%dC%d NPE LED%d",r,c,l),50,0,0);
      }
      for(int run=0; run<runT; run++){	
	for(int l=1; l<kNLED; l++){
	  NPE_f2[r][c][run][l-1]=NPE_f1[r][c][l][run];
	  if(NPE_f1[r][c][l][run]!=0){
	    if(NPE_f1[r][c][l][run+1]){ //To avoid large swings included in average, ignore nearest HV point to LED off
	      if(NPE_f1[r][c][l][run+1]!=0){ //Do not add saturated or LED off to histogram

		//Brute force fix problem with averages making max/min procedure impossible - resulting from large outliers.
		if((r==17&&c==1&&l==1&&run==0)||(r==18&&c==1&&l==1&&run==0)||(r==19&&c==1&&l==4&&run>0)||(r==21&&c==3&&l==1&&run>7)||(r==22&&c==3&&l==1&&run>8)) continue;
		
		NPEhist[r][c][l]->Fill(NPE_f1[r][c][l][run]); 
	      }
	    }
	  }
	}
  ...
  for(int c=144; c<(kNrows*kNcols); c++){
    ...
    for(int l=1; l<kNLED; l++){ //Start at one since at zero the histogram is empty
      
      avgNPE[c][l]=NPEhist[rval][cval][l]->GetMean();
      SDNPE[c][l]=NPEhist[rval][cval][l]->GetRMS();
  ...
  ofstream platFile;
  platFile.open("outFiles/plateaus.txt");
  time_t now2 = time(0);
  char *dt2 = ctime(&now2);
  double pDev = 0.2; //Percent deviation allowed from average to be considered inside "plateau."
  platFile << "#Plateau regions (+/-" << pDev*100 << "% from good average) obtained from LEDplots_v3 on " << dt2 << "#" << endl;

  //Read in values from Vanessa's spreadsheet
  for(int l=1; l<5; l++){ //Just look for led on (!0) and not saturated (!6)
    ifstream file(Form("setFiles/Vplat_l%d.txt",l));
    
    cout << "Getting Vanessa's plateaus for led " << l << ".." << endl;
	...
	continue;
      ...
      //cout << "Vplat range for channel " << n1 << " is " << VplatMin[n1][l] << " - " << VplatMax[n1][l] << "." << endl;
      
      n1++;
    }
  }
  
  //Look for plateau region with some rigor. Plateau defined within 20% of NPE mean. Does NOT yet deal with outliers in plateau.
  for(int r=12; r<kNrows; r++){
    for(int c=0; c<kNcols; c++){
      for(int l=1; l<kNLED-1; l++){
	int HVplatMax = 0;
	int HVplatMin = 10000;
	
	//cout << avgNPE[12*r+c][l] << endl;
	
	for(int run=0; run<runT; run++){	

	  //Brute force fix a few problem runs where the first point is abnormally high
	  if((r==17&&c==1&&l==1&&run==0)||(r==18&&c==1&&l==1&&run==0)||(r==19&&c==1&&l==4&&run==0)||(r==21&&c==3&&l==1&&run==0)||(r==22&&c==3&&l==1&&run==0)) cout << NPE_f1[r][c][l][run] << "  " << avgNPE[12*r+c][l] << "  " << NPE_f1[r][c][l][run] << "  " << avgNPE[12*r+c][l] << endl;
	  
	  if(NPE_f1[r][c][l][run]<(1.0+pDev)*avgNPE[12*r+c][l]&&NPE_f1[r][c][l][run]>(1.0-pDev)*avgNPE[12*r+c][l]){

	    if((r==17&&c==1&&l==1&&run==0)||(r==18&&c==1&&l==1&&run==0)||(r==19&&c==1&&l==4&&run==0)||(r==21&&c==3&&l==1&&run==0)||(r==22&&c==3&&l==1&&run==0)) cout << "GOT HERE" << endl;
	    
	    if(c>3&&c<8){
	      if(runHV_jlab[run]>HVplatMax) HVplatMax=runHV_jlab[run];
	      if(runHV_jlab[run]<HVplatMin) HVplatMin=runHV_jlab[run];
	    }else{
	      if(runHV_cmu[run]>HVplatMax) HVplatMax=runHV_cmu[run];
	      if(runHV_cmu[run]<HVplatMin) HVplatMin=runHV_cmu[run];
	    }	    
	  }
	}

	SplatMin[12*r+c][l]=HVplatMin;
	SplatMax[12*r+c][l]=HVplatMax;

	Sdif[l][12*r+c]=SplatMax[12*r+c][l]-SplatMin[12*r+c][l];
	Smean[l][12*r+c]=Sdif[l][12*r+c]/2.0+SplatMin[12*r+c][l];
	Vdif[l][12*r+c]=VplatMax[12*r+c][l]-VplatMin[12*r+c][l];
	Vmean[l][12*r+c]=Vdif[l][12*r+c]/2.0+VplatMin[12*r+c][l];

	//cout << "VplatMax[12*r+c][l]-VplatMin[12*r+c][l]=Vdif[l][12*r+c] : " << VplatMax[12*r+c][l] << " - " << VplatMin[12*r+c][l] << " = " << Vdif[l][12*r+c] << endl;

	if(Smean[l][12*r+c]>4000) cout << "SplatMax[12*r+c][l]-SplatMin[12*r+c][l]=Sdif[l][12*r+c] : " << SplatMax[12*r+c][l] << " - " << SplatMin[12*r+c][l] << " = " << Sdif[l][12*r+c] << endl;

	if(Smean[l][12*r+c]>4000) cout << "Row " << r << " Col " << c << " LED " << l << " bad plateau on S." << endl;
	
	//cout << "Means are (S  V): " << Smean[l][12*r+c] << "  " << Vmean[l][12*r+c] << endl;
	
	if(HVplatMin==10000) {
	  platFile << "Not enough data from row " << r << ", col " << c << ", and LED " << l << " to evaluate plateau." << endl;
	}else{
	  platFile << "Plateau region for row " << r << ", col " << c << ", and LED " << l << " is from " << HVplatMin << "-" << HVplatMax << "." << endl;
	}
	
	if(platFile){
	  platFile << "Difference from S to V channel " << 12*r+c << ": " << SplatMin[12*r+c][l] - VplatMin[12*r+c][l] << " to " << SplatMax[12*r+c][l] - VplatMax[12*r+c][l] << "." << endl;
	}

	//cout << "Difference from S to V channel " << 12*r+c << ": " << SplatMin[12*r+c][l] - VplatMin[12*r+c][l] << " to " << SplatMax[12*r+c][l] - VplatMax[12*r+c][l] << "." << endl;
	
      }
    }
  }
  
  for(int l=1; l<4; l++){ //Ignoring led 5 - too many outliers    
    
    TGraphErrors *G2 = new TGraphErrors(chan,channel,Smean[l],0,Sdif[l]);
    ...
    LEDpulse_HVcal->cd();
    G2->Write(Form("SEBplatLED%d",l));
    G2->Draw("AP");
    
    TGraphErrors *G3 = new TGraphErrors(chan,channel,Vmean[l],0,Vdif[l]);
    G3->SetMarkerColor(kWhite);
    G3->SetTitle("V Code");
    G3->SetMarkerStyle(23);
    G3->SetDrawOption("AP");
    G3->SetLineColor(kRed);
    LEDpulse_HVcal->cd();
    G2->Write(Form("VANplatLED%d",l));
    G2->Draw("AP");
    
    TMultiGraph *mg = new TMultiGraph();
    mg->Add(G2,"AP");
    mg->Add(G3,"AP");
    mg->SetTitle(Form("Plateaus, LED %d",l));
    mg->GetYaxis()->SetTitle("NPE Plateau Region (-V)");  //1 RAU approx 0.5 mV
    mg->GetYaxis()->CenterTitle();
    mg->GetXaxis()->SetTitle("Channel");
    mg->GetXaxis()->CenterTitle();
    mg->SetDrawOption("AP");
    LEDpulse_HVcal->cd();
    mg->Write(Form("DIFplatLED%d",l));
    mg->Draw("AP");
  }
  ...
  */  
  ...
  cout << "Completed loop over runs. Histograms written to outFiles/aggLEDPlotsV3.root. Fit parameters written to file PMTCalibrationData.txt. Alphas written to alphas.txt." << endl;
