//SSeeds 10.13.21 - Production - Code modified from Andrew Puckett's BigCal calibration script to calibrate BB energy deposition by PMT module. Tested with Eric Fuchey's digitized g4sbs data and with assumptions pending data from beam.
...
const int ncell = kNrowsSH*kNcolsSH; 
...
void bb_e_cal( int run = -1 ){
  ...
  //Declare outfile
  TFile *FFout = new TFile( Form("BBECal_%d.root",run), "RECREATE" );
  ...
  energy.clear();
  double gHV[kNrowsSH][kNcolsSH]={0};
  double gAlphas[kNrowsSH][kNcolsSH]={0};
  ...
  TFile *_file0 = TFile::Open("/volatile/halla/sbs/seeds/rootfiles/replayed_simdigtest_2_20211004.root");
  ...
  gmn_rec_tree *T = new gmn_rec_tree(C);
    ...
    T->SetBranchAddress( "sbs.hcal.a", a );
    T->SetBranchAddress( "sbs.hcal.a_amp", a_amp );
    T->SetBranchAddress( "sbs.hcal.a_p", a_p );
    T->SetBranchAddress( "sbs.hcal.a_c", a_c );
    T->SetBranchAddress( "sbs.hcal.a_amp_p", T->a_amp_p );
    T->SetBranchAddress( "sbs.hcal.tdc", T->tdc );
    T->SetBranchAddress( "sbs.hcal.ped", T->ped );
    T->SetBranchAddress( "sbs.hcal.adcrow", T->row );
    T->SetBranchAddress( "sbs.hcal.adccol", T->col );
    T->SetBranchStatus( "Ndata.sbs.hcal.adcrow", 1 );
    T->SetBranchAddress( "Ndata.sbs.hcal.adcrow", &T->ndata );
    
    // Add clustering branches
    T->SetBranchAddress("sbs.hcal.clus.id",T->cid); //ID of highest energy block in cluster
    T->SetBranchAddress("sbs.hcal.clus.row",T->crow);
    T->SetBranchAddress("sbs.hcal.clus.col",T->ccol);
    T->SetBranchAddress("sbs.hcal.clus.e",T->ce);
    T->SetBranchAddress("sbs.hcal.clus.eblk",T->ceblk); //Energy highest energy block in cluster
    T->SetBranchAddress("sbs.hcal.clus.nblk",T->cnblk);
    T->SetBranchAddress("sbs.hcal.nclus",T->nclus);
    T->SetBranchAddress("sbs.hcal.nblk",T->nblk);
    T->SetBranchAddress("sbs.hcal.clus_blk.id",T->cblkid);
    
    // Add BB cal vars
    T->SetBranchAddress("bb.tr.px",T->BBtr_px);
    T->SetBranchAddress("bb.tr.py",T->BBtr_py);
    T->SetBranchAddress("bb.tr.pz",T->BBtr_pz);
    T->SetBranchAddress("bb.tr.p",T->BBtr_p); //Magnitude momentum BB
    T->SetBranchAddress("bb.tr.chi2",T->BBtr_chi2); //Chi^2 per track in BB
    T->SetBranchAddress("bb.tr.n",T->bb_tr_n); //number of tracks in BB
    */
  cout << "Opened up tree with nentries=" << ENTRIES << endl;
    //}
  ...
  //cout << "hitlist size e ? " << hitlist.size() << endl;
  //cout << "energy size e ? " << energy.size() << endl;
  /*
  ...
  */
  ...
  //cout << "hitlist size f ? " << hitlist.size() << endl;
  //cout << "energy size f ? " << energy.size() << endl;
  /*
  ...
  cout << "hitlist size d ? " << hitlist.size() << endl;
  cout << "energy size d ? " << energy.size() << endl;
  ...
  */
  //cout << "hitlist size c ? " << hitlist.size() << endl;
  //cout << "energy size c ? " << energy.size() << endl;
  ...
  while( T->GetEntry( mevent++ ) ){ 
    ...
    //cout << "hitlist size b ? " << hitlist.size() << endl;
    //cout << "energy size b ? " << energy.size() << endl;
    if( mevent%1000 == 0 ) cout << "Preliminary loop: " << mevent << "/" << ENTRIES << endl;
    ...
    for(int elem=0; elem<track_tot; elem++){
      ...
      BBtr_chi2->Fill(T->bb_tr_chi2[elem]);
    ...
    BBbesttr->Fill(track);
    BBtr_bestchi2->Fill(T->bb_tr_chi2[track]);
    
    TVector3 lVep(T->bb_tr_px[track],T->bb_tr_py[track],T->bb_tr_pz[track]);
    //TVector3 lVep(T->bb_tr_px[0],T->bb_tr_py[0],T->bb_tr_pz[0]); //3Vector of scattered electron in lab frame, zero element most probable track
    
    if(debug==1) cout << "px = " << T->bb_tr_px[0] << ", py = " << T->bb_tr_py[0] << ", pz = " << T->bb_tr_pz[0] << ", lVep.Mag() = " << lVep.Mag() << "." << endl;
    ...
    double theta = acos((T->bb_tr_pz[0])/p_ep) * 180.0 / PI;
    Theta_e->Fill(theta);
    
    double phi_e = asin(T->bb_tr_py[0]/(p_ep*sin(theta)));
    Phi_e->Fill(phi_e);
    ...
    double phi_p = acos((E_e-T->bb_tr_pz[0])/p_p) * 180.0 / PI;
    Phi_p->Fill(phi_p);
    ...
    if(W2_ep > 0 && W2_ep < 2){ //Totally arbitrary for testing inelastic data until we have elastic data
      hitlist.push_back( int(mevent) );
      energy.push_back( E_ep );
    }
    //cout << mevent << endl;
    //cout << "hitlist size a ? " << hitlist.size() << endl;
    //cout << "energy size a ? " << energy.size() << endl;
    //cout << W2_ep << endl;
  }
  ...
  */
  ...
  while( T->GetEntry( hitlist[nevent++] ) ){ 
    ...
    if(hitlist[nevent]>ENTRIES) continue;
    ...
    for( int m = 0; m < T->Ndata_bb_sh_adcrow; m++ ) {
	...
	continue;
      ...
      adc_p[r][c] = T->bb_sh_a_p[m]; 
      amp[r][c] = T->bb_sh_a_amp[m];
      e[r][c] = T->bb_sh_a_c[m];  //Assuming this is the previously calibrated ADC
    ...
    int best = T->bb_sh_clus_id[0]; //index in the array of the "best" block in the best cluster found in this event. Currently only one cluster is recorded per event

    if( debug==1 ) cout << "best = " << best << endl;

    cout << best << endl;

    int ncellclust = T->bb_sh_nblk; //total number of hits in the "best" cluster, default to 9 (3x3) for now
    ...
    if( best >= 0 && ncellclust >= 1 && saturated == 0 ){ // require at least 2 hits in this cluster to use as a "good" event for calibration purposes:
      ...
      double E_p = energy[nevent]; //Energy of e' from first loop
	...
	for( int ihit=0; ihit<ncellclust; ihit++ ){ //outer loop over all cells in the cluster: 
	  ...
	  int cell_i = T->bb_sh_clus_blk_id[ihit];
	  ...
	  if(nevents[cell_i] > 4000) continue; //Added to keep statistics similar across all channels

	  for(int jhit=0; jhit<ncellclust; jhit++ ){ //inner loop over all cells in the cluster: 

	    int cell_j = T->bb_sh_clus_blk_id[jhit];
	    ...
	    if( cell_i >= kNrowsSH*kNcolsSH || cell_j >= kNrowsSH*kNcolsSH) {
  ...
  */
  ...
  constants_file.open( Form( "/work/halla/sbs/seeds/outFiles/BBECalParams_%d.txt", run ) );
  HVOutFile.open( Form( "/work/halla/sbs/seeds/outFiles/BBECalHVTargets_run%d.txt", run ) );
  ...
  double E_targ = 571; //Average peak for 14 MeV is 8 pC, so our conversion is 8/0.014 pC/GeV to get GeV. Similarly 14 MeV is 18 mV for maxADC (for reference)
  ...
  for(int i=0; i<ncell; i++){
	...
	cout << "Error: looping outside of SH geometry" << endl;
      }
